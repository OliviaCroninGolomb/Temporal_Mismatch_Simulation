...
```{r}
#Make character date into a datetime object. Extract the date, time, year, and day. Add an index column.
...
#Extract the noon sample, as that will be the field sample proxy that the values from the increasing time intervals will be compared against.
```{r}
...
#Plot mean absolute deviation of chl range within defined intervals
```{r}
...
#Field Sample Time Specific Intervals
```{r}
...
#Error Stats for Each Time Interval
```{r}
percent <- 0.2
...
#Consecutive Days Chlorophyll Time Series
```{r}
#Field samples
...
#Consecutive Days Chlorophyll Time Series Simple
```{r}
...
#Consecutive Days Chlorophyll Time Series Midpoint Day Aug 14
```{r}
...
#Consecutive Days Wind Speed Time Series
```{r}
